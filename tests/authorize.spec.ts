...
describe('Authorize', () => {
    const session_token_code = token.session_token_code
    const verifier = "OwaTAOolhambwvY3RXSD-efxqdBEVNnQkc0bBJ7zaak"
    const session_token = new JWT<Token.SessionToken>(token.session_token);

    it('Session Token', async () => {
        await OAuth.authorize(session_token_code, verifier)
    }, 10000)
